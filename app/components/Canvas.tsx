...
export function Canvas({ onNodeDoubleClick, canvasWrapRef }: Props) {
  ...
  const handleBgClick = useCallback(() => {
    if (state.mode === 'connect' && state.connectStep === 1) {
  ...
  return (
    ...
    >
      ...
      <div
        style={{ transform: `translate(${state.panX}px,${state.panY}px) scale(${state.scale})`, transformOrigin: '0 0', width: 3000, height: 3000, position: 'relative' }}
        onClick={e => e.stopPropagation()}
      >
        <ConnectionLayer />
        {state.diagram.nodes.map(node => (
          <NodeCard key={node.id} node={node} onDoubleClick={onNodeDoubleClick} />
        ))}
      </div>
      <Legend />
      <PropPanel />
