...
import { DiagramNode, Port, CONTAINER_TYPES, getPortPosition } from '../types/diagram';
...
export function NodeCard({ node, onDoubleClick }: Props) {
      ...
      {node.ports.map((port, idx) => {
        ...
        return (
          <button key={port.id} data-port={port.id}
            className={[
              'absolute w-3 h-3 rounded-full border-2 -translate-x-1/2 -translate-y-1/2 transition-all z-20',
              isConnectMode ? 'cursor-crosshair hover:scale-150' : 'cursor-default',
            ].join(' ')}
  ...
  if (isNttCloud) {
    ...
    return (
      <div
        className={['absolute select-none', isSelected ? 'z-20' : 'z-0'].join(' ')}
    ...
    return (
      <div
        ...
        style={{
          ...
          width: node.width, height: node.height ?? 200,
          background: node.bg + '44',
          borderColor: isSelected ? '#3b82f6' : node.color + '77',
          zIndex: isSelected ? 20 : 0, // コンテナは背面
        }}
        onMouseDown={handleMouseDown}
        onDoubleClick={e => { e.stopPropagation(); onDoubleClick(node.id); }}
      >
        <div className="px-2 py-1 text-[11px] font-semibold flex items-center gap-1.5"
          style={{ color: node.color, background: node.bg + 'AA', borderRadius: '6px 6px 0 0' }}>
  ...
  return (
    ...
    >
      {/* 上ポート */}
      {side === 'top' && node.ports.length > 0 && (
        <div className="flex justify-around px-3 pt-1.5 pb-0 gap-1">
          {node.ports.map(port => (
            <div key={port.id} className="flex flex-col items-center gap-0.5">
              <PortDot port={port} />
              <span className="text-[8px] text-gray-400 leading-none">{port.label}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="px-2 py-1.5 flex items-center gap-1.5 font-semibold"
        style={{
          ...
          borderRadius: side === 'top' ? '0 0 0 0' : '6px 6px 0 0',
        }}>
        <span className="w-4 h-4 rounded text-[9px] font-bold flex items-center justify-center flex-shrink-0"
          style={{ background: node.color, color: node.bg }}>{node.label[0]}</span>
      ...
      {/* 下ポート */}
      {side === 'bottom' && node.ports.length > 0 && (
        <div className="flex justify-around px-3 pb-1.5 pt-0 gap-1">
          {node.ports.map(port => (
            <div key={port.id} className="flex flex-col items-center gap-0.5">
              <span className="text-[8px] text-gray-400 leading-none">{port.label}</span>
              <PortDot port={port} />
